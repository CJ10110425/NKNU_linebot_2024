        ...
        if plugin_name not in [None, []]:
            for plugin in plugin_name:
                if PluginUtility.compare_plugin_name(plugin):
                    ...
                    register_plugins[plugin] = PluginUtility.setup_plugin_config(plugin)
                else:
                    print(f"Plugin_config {plugin} failed to register successfully")
                    register_plugins["plugin_default"] = PluginUtility.setup_plugin_config("plugin_default")
...
class LinebotUtility:
    ...
    def setup_linebot() -> dict:
        try:
            config_data = FileSystem.load_configuration()
